...
| `dataSources`               | `data-sources`                   | It allows defining the DBMS that will be used in the solution                                                                               | `"Mongo DB" \| "SQL Lite" \| "SQL Server"`                                       | `"SQL Server"` |
| `databaseName`              | `database-name`                  | Name of the database where we are going to persist the information of our KB                                                                | `string`                                                                         | `undefined`    |
| `frontEnd`                  | `front-end`                      | It allows selecting multiple generators for the front end                                                                                   | `"Android" \| "Apple" \| "WeChatMiniProgram" \| "Web (.NET)" \| "Web (Angular)"` | `"Web (.NET)"` |
| `isLocal`                   | `is-local`                       | Specify whether it is feasible to display information related to local configuration parameters                                             | `boolean`                                                                        | `false`        |
...
| `prototypingEnvironments`   | `prototyping-environments`       | It allows selecting the default environment that the KB will have (additional environments can be created later)                            | `".NET Framework" \| ".NET" \| "Java"`                                           | `".NET"`       |
